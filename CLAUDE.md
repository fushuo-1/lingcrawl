...
| **Extract** | 文本提取 — 两种模式：纯全文返回（免费）或 LLM 结构化提取 |
...
- 认证系统（Supabase、API Key）、计费系统（Stripe）、云版端点（Agent、Browser）
...
1. **先写端到端测试**（snips），断言你的成功条件
   ...
   - 测试门控：
     - 需要 fire-engine：`!process.env.TEST_SUITE_SELF_HOSTED`
     - 需要 AI：`!process.env.TEST_SUITE_SELF_HOSTED || process.env.OPENAI_API_KEY || process.env.OLLAMA_BASE_URL`
2. **写代码**实现功能
...
- **集成其他 AI 模型** — 替换或扩展 LLM 调用（见 `lib/generic-ai.ts`）
