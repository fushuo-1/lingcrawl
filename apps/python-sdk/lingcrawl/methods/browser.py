"""
Browser session methods for LingCrawl v2 API.
...
def browser_execute(
    ...
    *,
    ...
    resp = client.post(f"/browser/{session_id}/execute", body)
    if not resp.ok:
...
def delete_browser(
    ...
    Returns:
        BrowserDeleteResponse
    """
    resp = client.delete(f"/browser/{session_id}")
    if not resp.ok:
