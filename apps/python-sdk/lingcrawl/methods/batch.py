"""
Batch scraping functionality for LingCrawl v2 API.
"""
...
def start_batch_scrape(
    ...
    *,
    ...
    Raises:
        LingCrawlError: If the batch scrape operation fails to start
    """
    ...
    response = client.post("/batch/scrape", request_data, headers=headers)
...
def get_batch_scrape_status(
    ...
    Raises:
        LingCrawlError: If the status check fails
    """
    # Make the API request
    response = client.get(f"/batch/scrape/{job_id}")
...
def _fetch_all_batch_pages(
    ...
    while current_url:
        ...
        if not response.ok:
            ...
            import logging
            logger = logging.getLogger("lingcrawl")
...
def cancel_batch_scrape(
    ...
    Raises:
        LingCrawlError: If the cancellation fails
    """
    # Make the API request
    response = client.delete(f"/batch/scrape/{job_id}")
...
def wait_for_batch_completion(
    ...
    Raises:
        LingCrawlError: If the job fails or timeout is reached
...
def batch_scrape(
    ...
    *,
    ...
    Raises:
        LingCrawlError: If the batch scrape fails to start or complete
        TimeoutError: If timeout is reached
    """
...
def process_large_batch(
    ...
    Raises:
        LingCrawlError: If any chunk fails
    """
...
def get_batch_scrape_errors(client: HttpClient, job_id: str) -> CrawlErrorsResponse:
    ...
    """
    response = client.get(f"/batch/scrape/{job_id}/errors")
