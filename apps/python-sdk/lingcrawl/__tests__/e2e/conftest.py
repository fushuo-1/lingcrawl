...
def api_url():
    # Prefer TEST_URL, then LINGCRAWL_API_URL (for parity with JS), then legacy API_URL
    return (
        os.getenv("TEST_URL")
        or os.getenv("LINGCRAWL_API_URL")
        or os.getenv("API_URL")
        or "https://api.lingcrawl.dev"
    )
...
def _inject_client(request, api_identity, api_url):
    ...
    if inst is not None and not hasattr(inst, "client"):
        try:
            ...
            inst.client = LingCrawl(api_key=api_identity.get("apiKey", ""), api_url=api_url)
        except Exception:
            pass
    # For function-based modules that expect a module-level `lingcrawl` symbol
    mod = getattr(request, "module", None)
    if mod is not None and not hasattr(mod, "lingcrawl"):
        try:
            from lingcrawl import LingCrawl
            setattr(mod, "lingcrawl", LingCrawl(api_key=api_identity.get("apiKey", ""), api_url=api_url))
        except Exception:
            pass
