...
from .utils.error_handler import LingCrawlError
...
class LingCrawlClient:
    """
    Main LingCrawl v2 API client.

    This client provides a clean, modular interface to all LingCrawl functionality.
    """

    @staticmethod
    def _is_cloud_service(url: str) -> bool:
        return "api.lingcrawl.dev" in url.lower()

    def __init__(
        self,
        api_key: Optional[str] = None,
        api_url: str = "https://api.lingcrawl.dev",
        timeout: Optional[float] = None,
        max_retries: int = 3,
        backoff_factor: float = 0.5
    ):
        """
        Initialize the LingCrawl client.

        Args:
            api_key: LingCrawl API key (or set LINGCRAWL_API_KEY env var)
            api_url: Base URL for the LingCrawl API
        ...
        if self._is_cloud_service(api_url) and not api_key:
            raise ValueError(
                "API key is required for the cloud API. Set LINGCRAWL_API_KEY environment variable "
    ...
    def start_extract(
        ...
        *,
        ...
        .. deprecated::
            ...
            Review https://docs.lingcrawl.dev/developer-guides/usage-guides/choosing-the-data-extractor
