...
from lingcrawl import LingCrawl, Watcher, AsyncWatcher
...
def example_watcher() -> None:
    ...
    if not api_key or not api_url:
        raise ValueError("LINGCRAWL_API_KEY and LINGCRAWL_API_URL must be set")

    client = LingCrawl(api_key=api_key, api_url=api_url)

    # Start a small crawl job
    job = client.start_crawl("https://docs.lingcrawl.dev", limit=2)
