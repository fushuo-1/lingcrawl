"""
Example demonstrating pagination functionality in LingCrawl v2 SDK.
...
from lingcrawl.types import PaginationConfig

# Initialize the client
lingcrawl = LingCrawl(api_key="your-api-key")
...
crawl_result = lingcrawl.crawl("https://example.com", limit=100)
...
batch_result = lingcrawl.batch_scrape(urls)
...
# The next URL is opaque; pass it back to the SDK to fetch the next page.
print("\n=== Example 8: Manual batch scrape with pagination control ===")
batch_job = lingcrawl.start_batch_scrape(urls)
status = lingcrawl.get_batch_scrape_status(batch_job.id, pagination_config=PaginationConfig(auto_paginate=False))
print(f"Batch scrape documents: {len(status.data)}")
print(f"Next URL: {status.next}")
if status.next:
    next_page = lingcrawl.get_batch_scrape_status_page(status.next)
...
async def async_example():
    async_client = AsyncLingCrawl(api_key="your-api-key")
