 ...
 * Based on Node.js SDK patterns and tested against live lingcrawl.dev.
 * 
 * Run with: LINGCRAWL_API_KEY=fc-xxx gradle test --tests "com.lingcrawl.SearchTest"
 */
class SearchTest {
    ...
    void testSearchMinimal() {
        ...
        SearchData results = client.search("What is LingCrawl?");
    ...
    void testSearchWithScrapeOptions() {
        ...
        SearchData results = client.search("LingCrawl documentation",
                SearchOptions.builder()
                        .limit(2)
    ...
    @Test
    @EnabledIfEnvironmentVariable(named = "LINGCRAWL_API_KEY", matches = ".*\\S.*")
    void testSearchLingCrawlSpecific() {
        System.out.println("\n=== Test: Search for LingCrawl ===");
        
        SearchData results = client.search("LingCrawl web scraping API",
                SearchOptions.builder()
                        .limit(10)
                        .build());

        assertNotNull(results.getWeb(), "Web results should not be null");
        assertFalse(results.getWeb().isEmpty(), "Should find LingCrawl results");
        
        // Verify results contain LingCrawl-related content
        boolean hasLingCrawlContent = results.getWeb().stream()
                .anyMatch(result -> {
                    ...
                    return url.contains("lingcrawl") ||
                           title.contains("lingcrawl") ||
                           desc.contains("lingcrawl");
                });
        
        assertTrue(hasLingCrawlContent, "Results should mention LingCrawl");
        
        System.out.println("✓ LingCrawl search completed");
        System.out.println("  Total results: " + results.getWeb().size());
        System.out.println("  Results mentioning LingCrawl: ✓");
