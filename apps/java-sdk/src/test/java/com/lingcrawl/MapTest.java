...
import com.lingcrawl.models.MapData;
import com.lingcrawl.models.MapOptions;
 ...
 * 
 * Run with: LINGCRAWL_API_KEY=fc-xxx gradle test --tests "com.lingcrawl.MapTest"
 */
class MapTest {
    ...
    void testMapMinimal() {
        ...
        MapData data = client.map("https://docs.lingcrawl.dev");
    ...
    void testMapWithSkipSitemap() {
        ...
        MapData data = client.map("https://lingcrawl.dev",
                MapOptions.builder()
                        .sitemap("skip")
                        .limit(15)
                        .build());
    ...
    void testMapWithIncludeSubdomains() {
        ...
        boolean hasSubdomains = data.getLinks().stream()
                .anyMatch(link -> {
                    String url = link.get("url") != null ? link.get("url").toString() : "";
                    return url.contains("docs.lingcrawl.dev") || 
                           url.contains("api.lingcrawl.dev") ||
                           (url.contains(".lingcrawl.dev") && !url.contains("www.lingcrawl.dev"));
                });
    ...
    @Test
    @EnabledIfEnvironmentVariable(named = "LINGCRAWL_API_KEY", matches = ".*\\S.*")
    void testMapLingCrawlDocs() {
        System.out.println("\n=== Test: Map LingCrawl Documentation ===");
        ...
        System.out.println("✓ Mapped LingCrawl documentation");
        System.out.println("  Total links: " + data.getLinks().size());
