...
Java SDK for the [LingCrawl](https://lingcrawl.dev) v2 web scraping API.
...
1. Get your API key from [LingCrawl Dashboard](https://lingcrawl.dev)
2. Set it as an environment variable:
   ```bash
   export LINGCRAWL_API_KEY="fc-your-api-key-here"
   ```

3. **Or** add it to your shell profile for persistence:
   ```bash
   # For Zsh (macOS/Linux)
   echo 'export LINGCRAWL_API_KEY="fc-your-api-key-here"' >> ~/.zshrc
   source ~/.zshrc
   
   # For Bash
   echo 'export LINGCRAWL_API_KEY="fc-your-api-key-here"' >> ~/.bashrc
   source ~/.bashrc
   ```
...
```kotlin
implementation("com.lingcrawl:lingcrawl-java:1.0.0")
```
...
```xml
<dependency>
    <groupId>com.lingcrawl</groupId>
    <artifactId>lingcrawl-java</artifactId>
...
```java
// export LINGCRAWL_API_KEY=fc-your-api-key
...
```java
import com.lingcrawl.models.JsonFormat;
...
```java
SearchData results = client.search("lingcrawl web scraping",
...
AgentStatusResponse result = client.agent(
    AgentOptions.builder()
        .prompt("Find the pricing plans for LingCrawl and compare them")
        .build());
...
```java
LingCrawlClient client = LingCrawlClient.builder()
    .apiKey("fc-your-api-key")            // Required (or set LINGCRAWL_API_KEY env var)
    .apiUrl("https://api.lingcrawl.dev")  // Optional (or set LINGCRAWL_API_URL env var)
...
```bash
gradle jar
# Output: build/libs/lingcrawl-java-1.0.0.jar
```
...
# Now available as: com.lingcrawl:lingcrawl-java:1.0.0 in local Maven repository
```
...
E2E tests make real API calls and require a valid API key. These tests will be **skipped** if `LINGCRAWL_API_KEY` is not set:
...
gradle test --tests "com.lingcrawl.LingCrawlClientTest"
```
...
2. **Set Environment Variables:**
   ...
   export LINGCRAWL_API_URL="http://localhost:3002"
   ```
