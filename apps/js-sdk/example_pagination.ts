...
async function main() {
  const apiKey = process.env.LINGCRAWL_API_KEY || "fc-YOUR_API_KEY";
  const lingcrawl = new LingCrawl({ apiKey });
  ...
  const crawlAll = await lingcrawl.crawl("https://docs.lingcrawl.dev", { limit: 5 });
  ...
  const crawlStart = await lingcrawl.startCrawl("https://docs.lingcrawl.dev", { limit: 5 });
  ...
  const crawlSingle = await lingcrawl.getCrawlStatus(crawlJobId, { autoPaginate: false });
  ...
  const crawlLimited = await lingcrawl.getCrawlStatus(crawlJobId, {
    autoPaginate: true,
  ...
  const batchAll = await lingcrawl.batchScrape([
  ...
  // Use this when you need to control how many pages to fetch or want to process results incrementally
  const batchStart = await lingcrawl.startBatchScrape([
    "https://docs.lingcrawl.dev",
    "https://lingcrawl.dev",
  ], { options: { formats: ["markdown"] } });
  ...
  const batchSingle = await lingcrawl.getBatchScrapeStatus(batchJobId, { autoPaginate: false });
  ...
  const batchLimited = await lingcrawl.getBatchScrapeStatus(batchJobId, {
    autoPaginate: true,
    maxPages: 2,        // Only fetch 2 pages maximum
