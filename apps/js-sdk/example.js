...
import { LingCrawlClient } from './lingcrawl/src/v2/client.js';

async function run() {
  const apiKey = (globalThis.process && globalThis.process.env && globalThis.process.env.LINGCRAWL_API_KEY) || 'fc-YOUR_API_KEY';
  const client = new LingCrawlClient({ apiKey });
...
import LingCrawlApp from 'lingcrawl';
import { z } from 'zod';
...
const main = async () => {
  ...
  const extractResult = await app.extract(['https://lingcrawl.dev'], {
