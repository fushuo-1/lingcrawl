...
// Minimal usage of new LingCrawlClient. Replace with your API key before running.

// import LingCrawl from '@lingcrawl/lingcrawl-js';
import LingCrawl from './lingcrawl/src/index';

const run = async () => {
  const apiKey = (globalThis as any).process?.env?.LINGCRAWL_API_KEY || 'fc-YOUR_API_KEY';
  const apiUrl = (globalThis as any).process?.env?.LINGCRAWL_API_URL || 'https://api.lingcrawl.dev';
  const client = new LingCrawl({ apiKey, apiUrl });

  const doc = await client.scrape('https://docs.lingcrawl.dev', { formats: ['markdown'] });
  console.log('scrape:', !!doc.markdown);

  const crawl = await client.crawl('https://docs.lingcrawl.dev', { limit: 3, pollInterval: 1, timeout: 120 });
  ...
  const map = await client.map('https://lingcrawl.dev');
...
import LingCrawlApp, { CrawlStatusResponse, ErrorResponse } from 'lingcrawl';

const app = new LingCrawlApp({apiKey: "fc-YOUR_API_KEY"});

const main = async () => {

  // Scrape a website:
  const scrapeResult = await app.scrapeUrl('lingcrawl.dev');
  ...
  // Map a website:
  const mapResult = await app.mapUrl('https://lingcrawl.dev');
  ...
  // const extractResult = await app.extractUrls(['https://lingcrawl.dev'], {
