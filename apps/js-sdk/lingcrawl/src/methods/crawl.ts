...
export async function startCrawl(http: HttpClient, request: CrawlRequest): Promise<CrawlResponse> {
  ...
  try {
    const res = await http.post<{ success: boolean; id: string; url: string; error?: string }>("/crawl", payload);
...
export async function crawlParamsPreview(http: HttpClient, url: string, prompt: string): Promise<Record<string, unknown>> {
  ...
  try {
    const res = await http.post<{ success: boolean; data?: Record<string, unknown>; warning?: string }>("/crawl/params-preview", { url: url.trim(), prompt });
