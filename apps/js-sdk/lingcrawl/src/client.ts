 ...
 */
export interface LingCrawlClientOptions {
  /** API key (falls back to LINGCRAWL_API_KEY). */
  apiKey?: string | null;
  /** API base URL (falls back to LINGCRAWL_API_URL or https://api.lingcrawl.dev). */
  apiUrl?: string | null;
...
/**
 * LingCrawl v2 client. Provides typed access to all v2 endpoints and utilities.
 */

export class LingCrawlClient {
  private readonly http: HttpClient;

  private isCloudService(url: string): boolean {
    return url.includes('api.lingcrawl.dev');
  }
   ...
   */
  constructor(options: LingCrawlClientOptions = {}) {
    const apiKey = options.apiKey ?? process.env.LINGCRAWL_API_KEY ?? "";
    const apiUrl = (options.apiUrl ?? process.env.LINGCRAWL_API_URL ?? "https://api.lingcrawl.dev").replace(/\/$/, "");

    if (this.isCloudService(apiUrl) && !apiKey) {
      throw new Error("API key is required for the cloud API. Set LINGCRAWL_API_KEY env or pass apiKey.");
    }
   ...
   * Review https://docs.lingcrawl.dev/developer-guides/usage-guides/choosing-the-data-extractor to find a replacement.
   */
...
export default LingCrawlClient;
