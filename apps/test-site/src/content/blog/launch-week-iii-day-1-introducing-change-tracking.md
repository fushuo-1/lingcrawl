...
description: "LingCrawl's enhanced Change Tracking feature now provides detailed insights into webpage updates, including diffs and structured data comparisons."
pubDate: "Apr 14, 2025"
heroImage: "../../assets/blog/changeTracking.jpg"
category: "updates"
---

**Welcome to Launch Week III, Day 1! Today we’re excited to announce Change Tracking** — an enhanced LingCrawl feature that automatically detects and details changes on websites, now available in beta for all users.
...
LingCrawl’s Change Tracking feature integrates effortlessly into your existing workflows with two simple request methods—scrape and crawl. You must specify the `markdown` format in addition to `changeTracking`:

### Scrape Request Example:

```typescript
const scrapeResponse = await app.scrapeUrl("https://lingcrawl.dev", {
...
```typescript
const crawlResponse = await app.crawlUrl("https://lingcrawl.dev", {
...
When enabled, LingCrawl compares current scrapes against previous versions based on URL, team ID, and markdown format:
...
- **Learn more:** [Read the docs for `/scrape`](https://docs.lingcrawl.dev/features/change-tracking) and [the docs for `/crawl`](https://docs.lingcrawl.dev/features/crawl#change-tracking).
- **Get help:** Join our [Discord community](https://discord.gg/S7Enyh9Abh) or contact [help@lingcrawl.com](mailto:help@lingcrawl.com).

**Ready to track detailed content changes?** [Sign up for LingCrawl](https://lingcrawl.dev/signup) and start today.
