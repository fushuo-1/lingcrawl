...
//! Highly recommended to move all LingCrawl API calls to the backend (e.g. Next.js API route)
const LINGCRAWL_API_URL = "https://api.lingcrawl.dev"; // Replace with your actual API URL whether it is local or using LingCrawl Cloud
const LINGCRAWL_API_KEY = "fc-YOUR_API_KEY"; // Replace with your actual API key
...
export default function LingCrawlComponent() {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const endpoint = `${LINGCRAWL_API_URL}/v0/${
      ...
      const response = await fetch(endpoint, {
        method: "POST",
        headers: {
          Authorization: `Bearer ${LINGCRAWL_API_KEY}`,
      ...
      if (formData.crawlSubPages) {
        ...
        if (jobId) {
          const statusEndpoint = `${LINGCRAWL_API_URL}/v0/crawl/status/${jobId}`;
          let statusData: {
            status: string;
            data?: { url: string }[];
            current?: number;
  ...
  const handleScrapeSelected = async () => {
    ...
    for (const [index, url] of selectedUrls.entries()) {
      try {
        const response = await fetch(`${LINGCRAWL_API_URL}/v0/scrape`, {
          method: "POST",
          headers: {
            Authorization: `Bearer ${LINGCRAWL_API_KEY}`,
            "Content-Type": "application/json",
      ...
      <Card>
        <CardHeader className="flex items-start justify-between mb-0 pb-4">
          ...
          <div className="text-sm text-gray-500 w-11/12 items-center">
            ...
            their AI apps to web data with LingCrawl. Learn more on the{" "}
            <a
              href="https://docs.lingcrawl.dev/"
        ...
        <CardContent className="space-y-4">
          <form onSubmit={handleSubmit}>
            <div className="flex items-center space-x-2">
              <Input
                placeholder="https://www.lingcrawl.dev/"
                className="flex-grow"
                name="url"
                value={formData.url}
