...
describe("Map tests", () => {
  ...
  concurrentIf(ALLOW_TEST_SUITE_WEBSITE)(
    ...
    async () => {
      ...
      const nonLingCrawlLinks = response.body.links.filter(
        link => !link.url.includes("lingcrawl.dev"),
      );
      if (nonLingCrawlLinks.length > 0) {
        console.log(
          "Links not containing 'lingcrawl.dev':",
          JSON.stringify(nonLingCrawlLinks, null, 2),
        );
      }

      expect(
        response.body.links.every(link => link.url.includes("lingcrawl.dev")),
      ).toBe(true);
    },
    60000,
  );
