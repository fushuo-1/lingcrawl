...
describe("E2E Tests for API Routes", () => {
  ...
  describe("POST /v0/scrape", () => {
    ...
    it.concurrent(
      ...
      async () => {
        ...
        expect(response.body.data.metadata.description).toBe(
          "Welcome to Roast My Website, the ultimate tool for putting your website through the wringer! This repository harnesses the power of LingCrawl to scrape and capture screenshots of websites, and then unleashes the latest LLM vision models to mercilessly roast them. 🌶️",
        );
        expect(response.body.data.metadata.keywords).toBe(
          "Roast My Website,Roast,Website,GitHub,LingCrawl",
        );
    ...
    //     .set("Authorization", `Bearer ${config.TEST_API_KEY}`)
    //     .set("Content-Type", "application/json")
    //     .send({ url: "https://lingcrawl.dev", pageOptions: { waitFor: 7000 } });
    ...
    //   expect(response.body.data).toHaveProperty("metadata");
    //   expect(response.body.data).not.toHaveProperty("html");
    //   expect(response.body.data.content).toContain("🔥 LingCrawl");
    ...
    it.concurrent(
      ...
      async () => {
        ...
        const firstResponse = await request(TEST_URL)
          ...
          .send({ url: "https://docs.lingcrawl.dev" });
          ...
          expect.arrayContaining([
            expect.objectContaining({
              url: expect.stringContaining(
                "https://lingcrawl.dev/?ref=mendable+banner",
              ),
            }),
            expect.objectContaining({
  ...
  describe("POST /v0/crawlWebsitePreview", () => {
    ...
    //   expect(response.statusCode).toBe(403);
    //   expect(response.body.error).toContain("LingCrawl currently does not support social media scraping due to policy restrictions. We're actively working on building support for it.");
    // });
    ...
    it.concurrent(
      ...
      async () => {
        const response = await request(TEST_URL)
          ...
          .set("Authorization", `Bearer ${config.TEST_API_KEY}`)
          .set("Content-Type", "application/json")
          .send({ url: "https://lingcrawl.dev", timeout: 1000 });
