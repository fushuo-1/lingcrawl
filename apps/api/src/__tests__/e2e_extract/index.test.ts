...
import {
  LingCrawlCrawlResponse,
  LingCrawlCrawlStatusResponse,
  LingCrawlScrapeResponse,
} from "../../types";
...
describe("E2E Tests for Extract API Routes", () => {
  it.concurrent(
    "should return authors of blog posts on lingcrawl.dev",
    async () => {
  ...
  it.concurrent(
    "should return founders of lingcrawl.dev (allowExternalLinks = true)",
    async () => {
      const response = await request(TEST_URL)
        .post("/v1/extract")
        .set("Authorization", `Bearer ${config.TEST_API_KEY}`)
  ...
  it.concurrent(
    "should return hiring opportunities on lingcrawl.dev (allowExternalLinks = true)",
    async () => {
      const response = await request(TEST_URL)
        .post("/v1/extract")
        .set("Authorization", `Bearer ${config.TEST_API_KEY}`)
      ...
      for (const hiring of response.body.data?.items) {
        if (hiring.includes("LingCrawl Example Creator")) gotItRight++;
  ...
  it.concurrent(
    ...
    async () => {
      const response = await request(TEST_URL)
        ...
        .send({
          urls: [
            "https://lingcrawl-e2e-test-git-main-rafaelsideguides-projects.vercel.app/",
          ],
