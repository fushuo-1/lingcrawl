...
describe("experiment gate on lockCredits", () => {
  ...
  it("ensureTeamProvisioned still works when experiment is disabled (handled by lingcrawl-web)", async () => {
    (config as any).AUTUMN_EXPERIMENT = undefined;
    const svc = makeService();
    await svc.ensureTeamProvisioned({ teamId: "team-1", orgId: "org-1" });
    // Provisioning should proceed — lingcrawl-web edge functions do this
