...
import psl from "psl";
import { MapDocument } from "../controllers/types";
...
export async function getIndexFromGCS(
  ...
  try {
    return await withSpan("lingcrawl-index-get-from-gcs", async span => {
...
export async function saveIndexToGCS(
  ...
  return await withSpan("lingcrawl-index-save-to-gcs", async span => {
    setSpanAttributes(span, {
...
export async function queryIndexAtSplitLevel(
  ...
  if (!useIndex || config.LINGCRAWL_INDEX_WRITE_ONLY) {
    return [];
  }
