...
async function buildNuqPostgresImage(runtime: string): Promise<void> {
  ...
  const build = execForward(
    `${runtime}@build`,
    `${runtime} build -t lingcrawl-nuq-postgres:latest ${NUQ_POSTGRES_PATH}`,
  );
  await build.promise;
...
async function startNuqPostgresContainer(
  ...
  const start = execForward(
    `${runtime}@start`,
    `${runtime} run -d --name ${containerName} -p 5432:5432 -e POSTGRES_PASSWORD=${shellEscape(POSTGRES_PASSWORD)} -e POSTGRES_USER=${shellEscape(POSTGRES_USER)} -e POSTGRES_DB=${shellEscape(POSTGRES_DB)} lingcrawl-nuq-postgres:latest`,
  );
  await start.promise;
...
async function setupNuqPostgres(): Promise<Services["nuqPostgres"]> {
  ...
  const containerName = "lingcrawl-nuq-postgres";
...
async function setupNuqRabbitMQ(): Promise<Services["nuqRabbitMQ"]> {
  ...
  const containerName = "lingcrawl-nuq-rabbitmq";
