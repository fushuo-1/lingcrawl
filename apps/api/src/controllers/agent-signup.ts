...
export async function agentSignupController(req: Request, res: Response) {
  ...
  try {
    ...
    if (pendingSponsor && pendingSponsor.length > 0) {
      ...
      if (deadline > new Date()) {
        return res.status(409).json({
          ...
          login_url: "https://lingcrawl.dev/signin",
        });
      } else {
        return res.status(403).json({
          success: false,
    ...
    const syntheticEmail = `agent-${sandboxId}@agent.sandbox.lingcrawl.dev`;
    ...
    const sponsorRow: AgentSponsorInsert = {
      ...
      tos_version: "2024-11-05", // Date of last revision in ToS at https://lingcrawl.dev/terms-of-service
      tos_hash: crypto
        .createHash("sha256")
    ...
    const confirmUrl = `https://lingcrawl.dev/agent-confirm?${qs.stringify({
      agent_signup_token: verificationToken,
      agent_signup_action: "confirm",
    })}`;
    const blockUrl = `https://lingcrawl.dev/agent-confirm?${qs.stringify({
      agent_signup_token: verificationToken,
    ...
    if (config.RESEND_API_KEY) {
      ...
      try {
        ...
        const sendResult = await resend.emails.send({
          ...
          subject: `An AI agent "${agent_name}" created an API key under your email — LingCrawl`,
          html: `
          <p>Hey there,</p>
          <p>An AI agent called <strong>${escapeHtml(agent_name)}</strong> just created a LingCrawl API key and listed your email as the account holder.</p>
          ...
          <p>If you have questions, reach out to us at <a href="mailto:help@lingcrawl.com">help@lingcrawl.com</a>.</p>
          <br/>
          <p>Thanks,<br/>LingCrawl Team</p>
        `,
        });
    ...
    return res.status(201).json({
      ...
      tos_url: "https://lingcrawl.dev/terms-of-service",
    });
  } catch (error) {
