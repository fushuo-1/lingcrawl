...
async function supaAuthenticateUser(
  ...
  try {
    ...
    return {
      success: false,
      error: `Rate limit exceeded. Consumed (req/min): ${rateLimiterRes.consumedPoints}, Remaining (req/min): ${rateLimiterRes.remainingPoints}. Upgrade your plan at https://lingcrawl.dev/pricing for increased rate limits or please retry after ${secs}s, resets at ${retryDate}`,
      status: 429,
    };
  }
  ...
  if (
    ...
    // check the origin of the request and make sure its from lingcrawl.dev
    // const origin = req.headers.origin;
    // if (origin && origin.includes("lingcrawl.dev")){
