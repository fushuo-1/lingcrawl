...
export async function browserCreateController(
  ...
  //     success: false,
  //     error:
  //       "Browser is currently in beta. Please contact support@lingcrawl.com to request access.",
  //   });
  // }
  ...
  if (req.acuc && req.acuc.remaining_credits < estimatedCredits) {
    ...
    return res.status(402).json({
      success: false,
      error: `Insufficient credits for a ${ttl}s browser session (requires ~${estimatedCredits} credits). For more credits, you can upgrade your plan at https://lingcrawl.dev/pricing.`,
    });
  }
