        ...
        if (sponsor.status === "pending") {
          ...
          if (req.acuc.adjusted_credits_used >= UNVERIFIED_CREDIT_LIMIT) {
            return res.status(402).json({
              ...
              login_url: "https://lingcrawl.dev/signin",
              upgrade_url: "https://lingcrawl.dev/pricing",
            });
          }
        }
          ...
          return res.status(402).json({
            ...
            error:
              ...
              (currencyName === "credits"
                ? "https://lingcrawl.dev/pricing or try changing the request limit to a lower value"
                : "https://www.lingcrawl.dev/extract#pricing") +
              ".",
          });
        }
      }
  ...
  if (config.RESTRICTED_COUNTRIES?.includes(country.country)) {
    ...
    return res.status(403).json({
      ...
      error: isSelfHosted()
        ...
        : "Use of headers, actions, and the FIRE-1 agent is not allowed by default in your country. Please contact us at help@lingcrawl.com",
    });
  }
