...
import { extractLinks } from "@lingcrawl/lingcrawl-rs";
import {
  fetchRobotsTxt,
  createRobotsChecker,
...
import { filterLinks, filterUrl } from "@lingcrawl/lingcrawl-rs";
...
enum DenialReason {
  ...
  ROBOTS_TXT = "This URL is blocked by the website's robots.txt file, which instructs crawlers not to access this page. LingCrawl respects robots.txt by default. To crawl this URL anyway, set ignoreRobotsTxt: true in your crawl request (note: this may violate the website's crawling policies).",
  FILE_TYPE = "This URL points to a file type that LingCrawl does not crawl (e.g., images, videos, fonts, archives). LingCrawl automatically skips non-document file extensions like .png, .jpg, .mp4, .zip, .css, .js, etc.",
  ...
  SOCIAL_MEDIA = "This URL points to a social media platform or is an email link. LingCrawl automatically skips social media links and mailto: links during crawling.",
  EXTERNAL_LINK = "This URL points to a different domain than the one being crawled, and external links are disabled. By default, LingCrawl only crawls URLs on the same domain as the starting URL. To crawl external links, set allowExternalLinks: true in your crawl request.",
  SECTION_LINK = "This URL contains a section anchor (#) and points to a specific section of a page rather than a separate page. LingCrawl treats these as duplicates of the base URL and skips them to avoid crawling the same content multiple times.",
  NON_WEB_PROTOCOL = "This URL uses a non-web protocol (such as mailto:, tel:, ftp:, ssh:, file:, or telnet:) that LingCrawl cannot scrape. LingCrawl only supports HTTP and HTTPS protocols.",
}
...
export class WebCrawler {
      ...
      Object.entries(res.denialReasons).forEach(([key, value]) => {
        ...
        switch (value) {
          ...
          case "FILE_TYPE":
            ...
            fancyDenialReasons.set(
              key,
              `This URL points to a file with extension ".${extension}" that LingCrawl does not crawl. LingCrawl automatically skips non-document file extensions like .png, .jpg, .mp4, .zip, .css, .js, etc. to focus on web pages with textual content.`,
            );
            break;
          default:
      ...
      .filter(link => {
        ...
        const isAllowed =
          this.ignoreRobotsTxt || skipRobots
            ? true
            : ((this.robots.isAllowed(link, "LingCrawlAgent") ||
                this.robots.isAllowed(link, "LingCrawlAgent")) ??
              true);
  ...
  public importRobotsTxt(txt: string) {
    ...
    const delay =
      this.robots.getCrawlDelay("LingCrawlAgent") ||
      this.robots.getCrawlDelay("LingCrawlAgent");
