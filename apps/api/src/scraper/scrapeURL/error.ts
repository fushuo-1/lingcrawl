  ...
  constructor(public errorCode: string) {
    const errorExplanations: Record<string, string> = {
      ERR_TUNNEL_CONNECTION_FAILED:
        "LingCrawl encountered an internal proxy error while establishing the tunnel.",
      ERR_TIMED_OUT:
      ...
      ERR_PROXY_CONNECTION_FAILED:
        "LingCrawl encountered an internal proxy error while connecting to the proxy.",
    ...
    const followUp = isProxyError
      ? "This is an internal LingCrawl proxy error. Please retry or contact support."
...
export class UnsupportedFileError extends TransportableError {
  constructor(public reason: string) {
    super(
      ...
      `The URL returned a file type that LingCrawl cannot process: ${reason}. LingCrawl supports HTML web pages, PDFs, and common document formats. Binary files like images, videos, executables, and archives are not supported. If you expected this URL to return a web page, the server may be misconfigured or returning the wrong content type.`,
    );
  }
...
export class NoCachedDataError extends TransportableError {
  constructor() {
    super(
      ...
      "No cached data is available for this URL that meets your specified age requirements. This error occurs when you use the minAge parameter to request only cached data, but LingCrawl has no cached version of this URL (or no version within the specified age range). To resolve this, either remove the minAge parameter to allow a fresh scrape, or try again later after the URL has been scraped and cached.",
    );
  }
...
export class ZDRViolationError extends TransportableError {
  constructor(public feature: string) {
    super(
      ...
      `The feature "${feature}" is not available when using Zero Data Retention (ZDR) mode. ZDR mode ensures that no scraped content is stored on LingCrawl servers, but this limits certain features that require data storage (such as the index engine, certain proxy modes, or advanced processing). To use this feature, you need to disable ZDR mode. Contact support@lingcrawl.com if you need help.`,
    );
  }
