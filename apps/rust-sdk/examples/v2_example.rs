//! Example usage of LingCrawl v2 API.
//!
...
async fn main() {
    ...
    match result {
        Ok(response) => {
            if let Some(web) = response.data.web {
                for item in web {
                    match item {
                        lingcrawl::v2::SearchResultOrDocument::WebResult(r) => {
                            println!("Result: {} - {}", r.url, r.title.unwrap_or_default());
                        }
                        lingcrawl::v2::SearchResultOrDocument::Document(d) => {
    ...
    let result: Result<Option<CompanyInfo>, _> = client
        .agent_with_schema(
            vec!["https://lingcrawl.dev".to_string()],
