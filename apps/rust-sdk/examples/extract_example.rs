use lingcrawl::{extract::ExtractParams, LingCrawlApp};
use serde_json::json;
use std::error::Error;
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    let urls = vec![
        "https://www.lingcrawl.dev/".to_string(),
    ...
    match args.command {
        Examples::Basic => {
            ...
            for _ in 0..5 {
                let response = lingcrawl.get_extract_status(&response.id).await?;
