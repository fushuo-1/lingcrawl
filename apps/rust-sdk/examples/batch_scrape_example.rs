...
async fn create_lingcrawl_app() -> Result<LingCrawlApp, Box<dyn Error>> {
    let api_url = std::env::var("LINGCRAWL_API_URL")
        .expect("Please set the LINGCRAWL_API_URL environment variable");
    LingCrawlApp::new_selfhosted(api_url, None::<&str>).map_err(|e| e.into())
}
...
async fn main() -> Result<(), Box<dyn Error>> {
    let cli = Cli::parse();
    let lingcrawl = create_lingcrawl_app().await?;
    ...
    match cli.command {
        Commands::Basic => {
            ...
            match lingcrawl.map_url(url_one, None).await {
            ...
            test_multiple_urls(&lingcrawl, urls, &webhook_url).await?;
