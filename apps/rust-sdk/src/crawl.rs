//! Crawl endpoint for LingCrawl API v2.
...
impl Client {
    ...
    ///
    /// ```no_run
    /// use lingcrawl::{Client, CrawlOptions};
    ///
    /// #[tokio::main]
    ...
    pub async fn start_crawl(
    ...
    ) -> Result<CrawlResponse, LingCrawlError> {
        ...
        let response = self
            ...
            .await
            .map_err(|e| {
                LingCrawlError::HttpError(format!("Starting crawl of {:?}", url.as_ref()), e)
            })?;
    ...
    /// }
    /// ```
    pub async fn get_crawl_status(&self, id: impl AsRef<str>) -> Result<CrawlJob, LingCrawlError> {
        let response = self
            .client
            ...
            .await
            .map_err(|e| {
                LingCrawlError::HttpError(format!("Checking crawl status {}", id.as_ref()), e)
            })?;
    ...
    async fn get_crawl_status_next(&self, next: &str) -> Result<CrawlJob, LingCrawlError> {
        let response = self
            .client
            .get(next)
            .headers(self.prepare_headers(None))
            .send()
            .await
            .map_err(|e| LingCrawlError::HttpError(format!("Paginating crawl at {}", next), e))?;
    ...
    ///
    /// ```no_run
    /// use lingcrawl::{Client, CrawlOptions, SitemapMode};
    ///
    /// #[tokio::main]
    /// async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ///     let client = Client::new("your-api-key")?;
    ...
    pub async fn crawl(
        ...
        url: impl AsRef<str>,
        options: impl Into<Option<CrawlOptions>>,
    ) -> Result<CrawlJob, LingCrawlError> {
        let options = options.into().unwrap_or_default();
    ...
    async fn wait_for_crawl(
        ...
        loop {
            ...
            match status.status {
                ...
                JobStatus::Failed => {
                    return Err(LingCrawlError::CrawlJobFailed(
    ...
    pub async fn cancel_crawl(
        &self,
        id: impl AsRef<str>,
    ) -> Result<CancelCrawlResponse, LingCrawlError> {
        let response = self
            .client
            ...
            .await
            .map_err(|e| {
                LingCrawlError::HttpError(format!("Cancelling crawl {}", id.as_ref()), e)
            })?;
    ...
    pub async fn get_crawl_errors(
        &self,
        id: impl AsRef<str>,
    ) -> Result<CrawlErrorsResponse, LingCrawlError> {
        let response = self
            .client
            ...
            .await
            .map_err(|e| {
                LingCrawlError::HttpError(format!("Getting crawl errors {}", id.as_ref()), e)
            })?;
...
pub(crate) fn convert_document_to_pub(doc: Document) -> crate::document::Document {
...
mod tests {
        ...
        let mock = server
            ...
            .with_body(
                json!({
                    "success": true,
                    "id": "crawl-123",
                    "url": "https://api.lingcrawl.dev/v2/crawl/crawl-123"
                })
                .to_string(),
            )
            .create();
    ...
    async fn test_get_crawl_errors_with_mock() {
        ...
        let mock = server
            .mock("GET", "/crawl/crawl-123/errors")
            .with_status(200)
            .with_header("content-type", "application/json")
            .with_body(
                json!({
        ...
        let start_mock = server
            ...
            .with_body(
                json!({
                    "success": true,
                    "id": "crawl-456",
                    "url": "https://api.lingcrawl.dev/v2/crawl/crawl-456"
                })
                .to_string(),
            )
            .create();
