//! LingCrawl API v2 client.

use reqwest::Response;
use serde::de::DeserializeOwned;
use serde_json::Value;

use crate::error::{LingCrawlAPIError, LingCrawlError};
...
///     // Create a client for the LingCrawl cloud service
...
impl Client {
    /// Creates a new client for the LingCrawl cloud service.
    ///
    /// # Arguments
    ///
    /// * `api_key` - Your LingCrawl API key.
    ///
    /// # Errors
    ///
    ...
    /// Creates a new client for a self-hosted LingCrawl instance.
    ///
    /// # Arguments
    ///
    /// * `api_url` - The base URL of your LingCrawl instance.
    ...
    pub(crate) async fn handle_response<T: DeserializeOwned>(
        ...
        let response = response
            .text()
            .await
            .map_err(LingCrawlError::ResponseParseErrorText)
            .and_then(|response_json| {
                serde_json::from_str::<Value>(&response_json)
                    .map_err(LingCrawlError::ResponseParseError)
                    .inspect(|data| {
...
mod tests {
    ...
    fn test_url_builder() {
        ...
        assert_eq!(client.url("/scrape"), "https://api.lingcrawl.dev/v2/scrape");
    }

    #[test]
    fn test_url_normalization_trailing_slash() {
        // Cloud URL with trailing slash should still require API key
        let result = Client::new_selfhosted("https://api.lingcrawl.dev/", None::<&str>);
        assert!(result.is_err());

        // Should work with API key
        let client = Client::new_selfhosted("https://api.lingcrawl.dev/", Some("key")).unwrap();
        assert_eq!(client.api_url, "https://api.lingcrawl.dev");
