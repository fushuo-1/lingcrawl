...
use crate::{LingCrawlApp, LingCrawlError, API_VERSION};
...
impl LingCrawlApp {
    /// Extracts information from URLs using the LingCrawl API.
    ///
    ...
    ) -> Result<ExtractStatusResponse, LingCrawlError> {
    ...
    /// # Returns
    ///
    /// A response containing the extract job ID, or a LingCrawlError if the request fails.
    ///
    /// # Notes
    ///
    ...
    pub async fn async_extract(
        &self,
        params: impl Into<ExtractParams>,
    ) -> Result<ExtractResponse, LingCrawlError> {
        ...
        let response = self
            ...
            .send()
            .await
            .map_err(|e| LingCrawlError::HttpError("Initiating extract job".to_string(), e))?;
    ...
    /// # Returns
    ///
    /// A response containing the status of the extract job, or a LingCrawlError if the request fails.
    pub async fn get_extract_status(
        ...
        let response = self
            ...
            .await
            .map_err(|e| {
                LingCrawlError::HttpError(format!("Checking status of extract {}", id.as_ref()), e)
            })?;
        ...
        loop {
            ...
            match status_data.status.as_str() {
                ...
                "failed" => {
                    ...
                    break Err(LingCrawlError::APIError(
...
mod tests {
    ...
    async fn test_real_extract() {
        ...
        let app = LingCrawlApp::new_selfhosted(api_url, None::<&str>).unwrap();
    ...
    async fn test_async_extract_with_mock() {
        ...
        let app = LingCrawlApp::new_selfhosted(server.url(), Some("test_key")).unwrap();
    ...
    async fn test_extract_validation_errors() {
        let app = LingCrawlApp::new_selfhosted("https://example.com", Some("test_key")).unwrap();
