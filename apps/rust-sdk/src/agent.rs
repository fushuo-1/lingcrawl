//! Agent endpoint for LingCrawl API v2.
//!
...
impl Client {
    ...
    ///
    /// ```no_run
    /// use lingcrawl::{Client, AgentOptions};
    ///
    /// #[tokio::main]
    ...
    pub async fn start_agent(
    ...
    ) -> Result<AgentResponse, LingCrawlError> {
        ...
        let response = self
            ...
            .send()
            .await
            .map_err(|e| LingCrawlError::HttpError("Starting agent task".to_string(), e))?;
    ...
    pub async fn get_agent_status(
        &self,
        id: impl AsRef<str>,
    ) -> Result<AgentStatusResponse, LingCrawlError> {
        let response = self
            .client
            ...
            .await
            .map_err(|e| {
                LingCrawlError::HttpError(format!("Getting agent status {}", id.as_ref()), e)
            })?;
    ...
    ///
    /// ```no_run
    /// use lingcrawl::{Client, AgentOptions, AgentModel};
    /// use serde_json::json;
    ///
    /// #[tokio::main]
    /// async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    /// }
    /// ```
    pub async fn cancel_agent(&self, id: impl AsRef<str>) -> Result<bool, LingCrawlError> {
        let response = self
            .client
            ...
            .await
            .map_err(|e| {
                LingCrawlError::HttpError(format!("Cancelling agent {}", id.as_ref()), e)
            })?;
    ...
    pub async fn agent_with_schema<T: serde::de::DeserializeOwned>(
        ...
        schema: Value,
    ) -> Result<Option<T>, LingCrawlError> {
        let options = AgentOptions {
            urls: Some(urls),
        ...
        match result.data {
            Some(data) => {
                let typed: T =
                    serde_json::from_value(data).map_err(LingCrawlError::ResponseParseError)?;
                Ok(Some(typed))
            }
            None => Ok(None),
        }
