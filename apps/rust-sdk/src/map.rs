//! Map endpoint for LingCrawl API v2.
...
impl Client {
    ...
    ///
    /// ```no_run
    /// use lingcrawl::{Client, MapOptions, SitemapMode};
    ///
    /// #[tokio::main]
    ...
    pub async fn map(
    ...
    ) -> Result<MapResponse, LingCrawlError> {
        let body = MapRequest {
        ...
        let response = self
            ...
            .send()
            .await
            .map_err(|e| LingCrawlError::HttpError(format!("Mapping {:?}", url.as_ref()), e))?;
    ...
    pub async fn map_urls(
        ...
        url: impl AsRef<str>,
        options: impl Into<Option<MapOptions>>,
    ) -> Result<Vec<String>, LingCrawlError> {
