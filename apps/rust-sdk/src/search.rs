//! Search endpoint for LingCrawl API v2.
...
impl Client {
    ...
    ///
    /// ```no_run
    /// use lingcrawl::{Client, SearchOptions, SearchSource};
    ///
    /// #[tokio::main]
    ...
    ///         match result {
    ///             lingcrawl::SearchResultOrDocument::WebResult(r) => {
    ///                 println!("URL: {}", r.url);
    ///             }
    ///             lingcrawl::SearchResultOrDocument::Document(d) => {
    ...
    pub async fn search(
    ...
    ) -> Result<SearchResponse, LingCrawlError> {
        ...
        let response = self
            ...
            .await
            .map_err(|e| {
                LingCrawlError::HttpError(format!("Searching for {:?}", query.as_ref()), e)
            })?;
    ...
    pub async fn search_and_scrape(
        ...
        query: impl AsRef<str>,
        limit: u32,
    ) -> Result<Vec<Document>, LingCrawlError> {
        let options = SearchOptions {
            limit: Some(limit),
