//! Scrape endpoint for LingCrawl API v2.
...
impl Client {
    ...
    ///
    /// ```no_run
    /// use lingcrawl::{Client, ScrapeOptions, Format};
    ///
    /// #[tokio::main]
    ...
    pub async fn scrape(
    ...
    ) -> Result<Document, LingCrawlError> {
        ...
        let response = self
            ...
            .send()
            .await
            .map_err(|e| LingCrawlError::HttpError(format!("Scraping {:?}", url.as_ref()), e))?;
