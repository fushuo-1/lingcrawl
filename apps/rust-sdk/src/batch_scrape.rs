//! Batch scrape endpoint for LingCrawl API v2.
...
impl Client {
    ...
    ///
    /// ```no_run
    /// use lingcrawl::{Client, BatchScrapeOptions};
    ///
    /// #[tokio::main]
    ...
    pub async fn start_batch_scrape(
    ...
    ) -> Result<BatchScrapeResponse, LingCrawlError> {
        ...
        let response = self
            ...
            .send()
            .await
            .map_err(|e| LingCrawlError::HttpError("Starting batch scrape".to_string(), e))?;
    ...
    pub async fn get_batch_scrape_status(
        &self,
        id: impl AsRef<str>,
    ) -> Result<BatchScrapeJob, LingCrawlError> {
        let response = self
            .client
    ...
    async fn get_batch_scrape_status_next(
        ...
        let response = self
            ...
            .await
            .map_err(|e| {
                LingCrawlError::HttpError(format!("Paginating batch scrape at {}", next), e)
            })?;
    ...
    ///
    /// ```no_run
    /// use lingcrawl::{Client, BatchScrapeOptions, ScrapeOptions, Format};
    ///
    /// #[tokio::main]
    /// async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ///     let client = Client::new("your-api-key")?;
    ...
    pub async fn get_batch_scrape_errors(
        ...
        let response = self
            ...
            .await
            .map_err(|e| {
                LingCrawlError::HttpError(format!("Getting batch scrape errors {}", id.as_ref()), e)
            })?;
...
fn convert_batch_job_to_crawl_status(job: BatchScrapeJob) -> crate::crawl::CrawlStatus {
    crate::crawl::CrawlStatus {
        ...
        data: job
            .data
            .into_iter()
            .map(|d| super::crawl::convert_document_to_pub(d))
            .collect(),
    }
}
...
impl super::crawl::CrawlJob {
    ...
    pub(crate) fn _convert_document(doc: Document) -> crate::document::Document {
        super::crawl::convert_document_to_pub(doc)
    }
}
...
mod tests {
        ...
        let mock = server
            ...
            .with_body(
                json!({
                    "success": true,
                    "id": "batch-123",
                    "url": "https://api.lingcrawl.dev/v2/batch/scrape/batch-123"
                })
                .to_string(),
            )
            .create();
    ...
    async fn test_get_batch_scrape_status_with_mock() {
        ...
        let mock = server
            .mock("GET", "/batch/scrape/batch-123")
            .with_status(200)
            .with_header("content-type", "application/json")
            .with_body(
                json!({
        ...
        let mock = server
            ...
            .with_body(
                json!({
                    "success": true,
                    "id": "batch-456",
                    "url": "https://api.lingcrawl.dev/v2/batch/scrape/batch-456",
    ...
    async fn test_batch_scrape_sync() {
        ...
        let start_mock = server
            ...
            .with_body(
                json!({
                    "success": true,
                    "id": "batch-789",
                    "url": "https://api.lingcrawl.dev/v2/batch/scrape/batch-789"
                })
                .to_string(),
            )
            .create();
        ...
        let status_mock = server
            .mock("GET", "/batch/scrape/batch-789")
            .with_status(200)
            .with_header("content-type", "application/json")
            .with_body(
                json!({
    ...
    async fn test_get_batch_scrape_errors() {
        ...
        let mock = server
            .mock("GET", "/batch/scrape/batch-123/errors")
            .with_status(200)
            .with_header("content-type", "application/json")
            .with_body(
                json!({
