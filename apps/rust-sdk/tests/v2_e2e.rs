//! End-to-end tests for LingCrawl v2 API.
//!
//! These tests require the following environment variables:
//! - API_URL: The LingCrawl API URL
...
async fn test_v2_search_with_options() {
    ...
    let response = client
        .search("lingcrawl web scraping", options)
        .await
...
async fn test_v2_crawl_async() {
    ...
    assert!(
        matches!(
            status.status,
            lingcrawl::v2::JobStatus::Scraping | lingcrawl::v2::JobStatus::Completed
        ),
...
async fn test_v2_crawl_sync() {
    ...
    assert!(
        job.status == lingcrawl::v2::JobStatus::Completed,
...
fn test_v2_client_creation() {
    ...
    let cloud_no_key = Client::new_selfhosted("https://api.lingcrawl.dev", None::<&str>);
