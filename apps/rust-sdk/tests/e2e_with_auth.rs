use dotenvy::dotenv;
use lingcrawl::scrape::{JsonOptions, ScrapeFormats, ScrapeOptions};
use lingcrawl::{LingCrawlApp, LingCrawlError};
use serde_json::json;
use std::env;
...
//     let app = LingCrawlApp::new_selfhosted(api_url, api_key).unwrap();
//     let unsupported_url = "https://facebook.com/fake-test";
//     let result = app.scrape_url(unsupported_url, None).await;

//     assert_matches!(
//         result,
//         Err(e) if e.to_string().contains("LingCrawl currently does not support social media scraping due to policy restrictions")
//     );
// }
...
async fn test_successful_response_with_valid_preview_token() {
    ...
    let api_url = env::var("API_URL").unwrap();
    let app =
        LingCrawlApp::new_selfhosted(api_url, Some(env::var("PREVIEW_TOKEN").unwrap())).unwrap();
    let result = app
...
fn test_api_key_requirements() {
    ...
    match (api_url.contains("api.lingcrawl.dev"), api_key) {
        (false, _) => {
            let result = LingCrawlApp::new_selfhosted(&api_url, None::<String>);
            assert!(
                result.is_ok(),
                "Local setup failed: {:?}",
                result.err().unwrap()
        ...
        (true, None) => {
            ...
            assert!(matches!(
                result,
                Err(LingCrawlError::APIError(msg, _)) if msg == "Configuration"
            ));
        }
        (true, Some(key)) => {
            let result = LingCrawlApp::new_selfhosted(&api_url, Some(&key));
            assert!(result.is_ok());
        }
    }
}
