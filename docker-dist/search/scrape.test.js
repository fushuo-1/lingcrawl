"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
jest.mock("uuid", () => ({
    v7: jest.fn(() => "job-1"),
}));
const scrape_1 = require("./scrape");
const job_priority_1 = require("../lib/job-priority");
const scrape_worker_1 = require("../services/worker/scrape-worker");
jest.mock("../lib/job-priority", () => ({
    getJobPriority: jest.fn().mockResolvedValue(10),
}));
jest.mock("../services/worker/scrape-worker", () => ({
    processJobInternal: jest.fn().mockResolvedValue({
        markdown: "body",
        metadata: { creditsUsed: 1, statusCode: 200, proxyUsed: "basic" },
    }),
}));
describe("scrapeSearchResults", () => {
    it("preserves billing metadata on spawned scrape jobs", async () => {
        await (0, scrape_1.scrapeSearchResults)([
            {
                url: "https://example.com",
                title: "Example",
                description: "Desc",
            },
        ], {
            teamId: "team-1",
            origin: "api",
            timeout: 60_000,
            scrapeOptions: {},
            apiKeyId: 123,
            requestId: "req-1",
            billing: { endpoint: "agent" },
        }, { debug: jest.fn(), info: jest.fn(), error: jest.fn() }, null);
        expect(job_priority_1.getJobPriority).toHaveBeenCalledWith({
            team_id: "team-1",
            basePriority: 10,
        });
        expect(scrape_worker_1.processJobInternal).toHaveBeenCalledWith(expect.objectContaining({
            data: expect.objectContaining({
                billing: { endpoint: "agent" },
            }),
        }));
    });
});
//# sourceMappingURL=scrape.test.js.map